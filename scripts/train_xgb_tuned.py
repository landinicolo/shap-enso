"""Tune and train XGBoost ENSO forecast models.

This is the tuned counterpart of ``scripts/train_xgb.py``. It imports
``src.models.xgb_model_tuned`` and writes all outputs under a tuned experiment
folder, leaving baseline ``data/models/xgb`` untouched.
...
python scripts/train_xgb_tuned.py \
    ...
    --max-trials 256 \
    --selection-metric r2 \
    --run-output-dir data/tuned_wide/xgb/lead06_regression
"""
...
from src.models.xgb_model_tuned import ENSOXGBModel
...
def main(
    ...
    if run_output_dir is None:
        run_dir = Path(cfg["experiment"]["output_dir"]) / "data" / "tuned_wide" / "xgb" / f"lead{lead:02d}_{task}"
    else:
    ...
    final_cfg = patch_xgb_cfg(cfg, best_params)
    model = ENSOXGBModel(final_cfg, lead, task)
    val_metrics = model.fit(X_tr, y_tr_fit, X_val, y_val_fit, list(feat_names))
    ...
    metrics_path = dirs["metrics"] / f"xgb_lead{lead:02d}_{task}_metrics.json"
...
def patch_xgb_cfg(cfg: dict, params: dict[str, Any]) -> dict:
    cfg_t = copy.deepcopy(cfg)
    xp = cfg_t["model"].setdefault("xgb", {})
...
def tune_xgb(
    ...
    with open(tuning_dir / f"xgb_lead{lead:02d}_{task}_tuning_grid.json", "w") as f:
    ...
    log.info("XGB tuning: %d candidates, %d CV folds, selection=%s", len(candidates), len(folds), metric_col)
    ...
    for trial_id, params in enumerate(candidates, start=1):
        ...
        for fold_id, (tr_idx, va_idx) in enumerate(folds, start=1):
            cfg_t = patch_xgb_cfg(cfg, params)
            model = ENSOXGBModel(cfg_t, lead, task)
            try:
                metrics = model.fit(X_dev[tr_idx], y_dev[tr_idx], X_dev[va_idx], y_dev[va_idx], feature_names)
                row = {
                    "trial_id": trial_id,
                    "fold_id": fold_id,
            ...
            pd.DataFrame(fold_rows).to_csv(tuning_dir / f"xgb_lead{lead:02d}_{task}_cv_folds.csv", index=False)

    fold_df = pd.DataFrame(fold_rows)
    summary_df = summarize_trials(fold_df, metric_col=metric_col, maximize=maximize)
    if summary_df.empty:
        raise RuntimeError("No valid XGB tuning trials completed.")

    fold_csv = tuning_dir / f"xgb_lead{lead:02d}_{task}_cv_folds.csv"
    summary_csv = tuning_dir / f"xgb_lead{lead:02d}_{task}_tuning_summary.csv"
    best_json = tuning_dir / f"xgb_lead{lead:02d}_{task}_best_params.json"
    ...
    make_tuning_plots(summary_df, fold_df, metric_col, figures_dir, prefix=f"xgb_lead{lead:02d}_{task}")
...
def summarize_trials(fold_df: pd.DataFrame, metric_col: str, maximize: bool) -> pd.DataFrame:
    ...
    } and not c.startswith("val_") and c not in {"best_iteration", "best_val_loss"}]
    metric_cols = [c for c in fold_df.columns if c.startswith("val_") or c in {"best_iteration", "best_val_loss"}]
    rows = []
...
def make_tuning_plots(summary_df: pd.DataFrame, fold_df: pd.DataFrame, metric_col: str, figures_dir: Path, prefix: str) -> None:
    ...
    if metric_col in fold_df.columns:
        ...
        plt.title("XGB tuning: fold scores")
        plt.tight_layout()
        plt.savefig(figures_dir / f"{prefix}_fold_scores.png", dpi=180)
        plt.close()

    param_cols = [c for c in ["max_depth", "learning_rate", "n_estimators", "min_child_weight", "reg_alpha", "reg_lambda", "subsample", "colsample_bytree"] if c in summary_df.columns]
    if param_cols:
    ...
    if {"max_depth", "learning_rate"}.issubset(summary_df.columns):
        pivot = summary_df.pivot_table(index="max_depth", columns="learning_rate", values=score_col, aggfunc="mean")
        if not pivot.empty:
            ...
            plt.title("XGB tuning: depth vs learning rate")
            plt.colorbar(label=score_col)
            plt.tight_layout()
            plt.savefig(figures_dir / f"{prefix}_depth_lr_heatmap.png", dpi=180)
            plt.close()
    ...
    if {"mean_val_r2", "mean_val_rmse"}.issubset(summary_df.columns):
        ...
        plt.title("XGB tuning: RMSE vs R2")
        plt.tight_layout()
...
def save_predictions(pred_dir: Path, lead: int, task: str, init_times, y_true: np.ndarray, y_pred: np.ndarray) -> None:
    ...
    out = pred_dir / f"xgb_lead{lead:02d}_{task}_test_predictions.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Tune and train XGBoost ENSO model")
    ...
    parser.add_argument("--max-trials", type=int, default=256, help="0 or negative means full grid")
